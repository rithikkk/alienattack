...
async function main() {
    ...
    const vertShaderSample =
        ...
        void main() {
            ...
            oNormal = normalize(vec3((normalMatrix * vec4(aNormal, 1.0)).xyz));
    ...
    const fragShaderSample =
        ...
        void main() {
            ...
            // Diffuse term
            // kd * ld * (N dot L)
            float diff = max(dot(normal, lightDirection), 0.0); // N dot L
            vec3 diffuse = mainLight.colour * diff; //ld * (N dot L)
            
            vec4 textureColour = vec4(1.0, 1.0, 1.0, 1.0);
            vec3 kd = vec3(1.0, 1.0, 1.0);

            // if applicable, apply texture
            if (samplerExists == 1){
                textureColour = texture(uTexture, oUV);
                kd = mix(diffuseVal, textureColour.rgb, 0.7);
                diffuse = kd;
            }
            else {
                diffuse =  diffuseVal*diffuse;
            }

            //specular term
            // ks * ls(white) * (N dot H)^n
            // H = V+L, V = camPos - fragPos
            vec3 vDirection = normalize(oCameraPosition - oFragPosition);
            vec3 hDirection = normalize(vDirection + lightDirection);

            float spec = max(dot(hDirection, normal), 0.0); // N dot H
            spec = pow(spec, nVal); // (N dot H)^n
            vec3 specular = specularVal * spec; // ks * (N dot H)^n 

            // Combine terms and apply alpha
